#	Google and iTunes Connect Reporter for AWS Lambda
...
fit rom there via API calls.
 ...
 awsBucketName in **app.py** - Example: 'YOUR_BUCKET_NAME' (this is the aws bucket name)
 awsFileName in **app.py** - Example: 'FILENAME.csv' (obvious, eh?)
 ```
 
 I tried to provide you with the neededl ibraries inside the projects "lib" folder.
 
## Usage examples
 Just upload this project to your lambda, install the needed libraries and it will automatically load the reports from 2 days before.
 If yo uneed to set up your S3 search for it in the amazon docs.
