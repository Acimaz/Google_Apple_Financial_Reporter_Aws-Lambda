...
class GoogleReporter:
    ...
    def __init__(self, reportDate):
        print 'Initializing GoogleReporter...'
        self.UpdateReportToDownloadName(reportDate[:-2])
        private_key = json.loads(open(self.json_file).read())['private_key']
        credentials = SignedJwtAssertionCredentials(self.client_email, private_key,'https://www.googleapis.com/auth/devstorage.read_only')
    ...
    def DownloadCsv(self, date):
        ...
        req = self.storage.objects().get_media(bucket=self.cloud_storage_bucket,
                                               object=self.report_to_download).execute()
